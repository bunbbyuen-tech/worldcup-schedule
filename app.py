...
def score_or_vs(m):
    if (m["finished"] or m["live"]) and m["home_goals"] is not None:
        s = f'<span class="score">{m["home_goals"]} - {m["away_goals"]}</span>'
        if m.get("note"):
            s += f'<span style="font-size:12px;color:#999;font-weight:700">（{m["note"]}）</span>'
        return s
...
# ----------------------------------------------------------------------------
with tab_ko:
    b = bracket.build_bracket()
    st.image(bracket.render_linear_png(b, starset), use_container_width=True)

    ko_order = [("r32", "32 強"), ("r16", "16 強"), ("qf", "8 強"),
                ("sf", "4 強"), ("third", "季軍戰"), ("final", "決賽")]
    ko = [m for m in matches if m["stage_key"] in dict(ko_order)]
    if ko:
        with st.expander("📋 淘汰賽各場詳情（時間／比數）"):
