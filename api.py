...
def _feeders(m):
    """Match numbers that feed into this match (from 'W<n>' placeholders)."""
    out = []
    for t in (m.get("team1"), m.get("team2")):
        mt = re.match(r"^W(\d+)$", t or "")
        if mt:
...
def _disp(m, now):
    """Compact display dict for one bracket match."""
    n = _normalise(m, now)
    short_date = "/".join(s.lstrip("0") for s in m["date"][5:].split("-")) if m.get("date") else ""
    return {
        "num": m.get("num"),
        ...
        "home": n["home"],
        "away": n["away"],
        "home_goals": n["home_goals"],
        "away_goals": n["away_goals"],
        "live": n["live"],
        "finished": n["finished"],
    }


_KO_ORDER = ["r32", "r16", "qf", "sf"]


def build_bracket():
    """Return the knockout bracket split into upper / lower halves.

    {upper: [{stage_key,label,matches:[...]}], lower: [...], final: m, third: m, source}
    Halves are derived from which semi-final each subtree feeds — no hard-coding.
    """
    data, source = _raw()
    now = datetime.now(timezone.utc)
    by_num = {m["num"]: m for m in data.get("matches", []) if m.get("num")}

    final = next((m for m in data.get("matches", []) if m.get("round") == "Final"), None)
    third = next((m for m in data.get("matches", []) if m.get("round") == "Match for third place"), None)
    if not final:
        return None
    semis = _feeders(final)  # [101, 102]
    ...
    def half(semi_num):
        acc = []
        subtree(semi_num, acc)
        disp = [_disp(m, now) for m in acc]
        rounds = []
        for key in _KO_ORDER:
            ms = sorted([d for d in disp if d["stage_key"] == key],
                        key=lambda d: (d["num"] or 0))
            if ms:
                rounds.append({"stage_key": key, "label": ms[0]["stage_label"], "matches": ms})
        return rounds

    return {
        "upper": half(semis[0]) if len(semis) > 0 else [],
        "lower": half(semis[1]) if len(semis) > 1 else [],
        "final": _disp(final, now),
        "third": _disp(third, now) if third else None,
        "source": source,
    }
